...
interface MouseDao {
    ...
    @Query("SELECT * FROM sm WHERE Code = :code ORDER BY mouseDateTimeCreated DESC LIMIT 100")
    ...
    @Query("SELECT * FROM sm WHERE localityID = :localityId AND Code IS NOT NULL AND (:currentTime - mouseDateTimeCreated) < :twoYears")
