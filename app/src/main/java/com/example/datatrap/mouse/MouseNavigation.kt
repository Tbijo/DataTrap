...
fun NavGraphBuilder.mouseNavigation(navController: NavHostController) {
    ...
    composable(
        ...
        MouseScreen(
            onEvent = { event ->
                when(event) {
                    MouseScreenEvent.OnCameraClick -> {
                        navController.navigateToCameraScreen( // TODO add mouseId
                            entityId = "",
    ...
    composable(
        ...
        RecaptureListScreen(
            onEvent = { event ->
                when(event) {
                    is RecaptureListScreenEvent.OnItemClick -> { // TODO mouseId, isRecapture = true,
                        navController.navigateToMouseScreen(
                            screenNavArgs = screenNavArgs.copy(
                                isRecapture = true,
